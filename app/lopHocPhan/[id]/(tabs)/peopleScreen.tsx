...
export default function PeopleScreen() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const lhpRes = await fetch(`http://192.168.1.104:3001/lophophan/${id}`);
      const lhp = await lhpRes.json();

      // 2. Fetch giảng viên từ maGV
      const gvRes = await fetch(
        `http://192.168.1.104:3001/giangvien?id=${lhp.maGV}`
      );
      ...
      const sv_lhpRes = await fetch(
        `http://192.168.1.104:3001/sinhvien_lhp?maLHP=${id}`
      );
      ...
      const svs = await Promise.all(
        sv_lhp.map(async (item: any) => {
          const res = await fetch(
            `http://192.168.1.104:3001/sinhvien?maSV=${item.maSV}`
          );
